...
Eigen::Matrix3d vec2SO3(const Eigen::Vector3d& rot_vec) {
    double mag = 0;
...
Eigen::Matrix4d est2SE3(const Eigen::Vector3d& rot_vec, const Eigen::Vector3d& ts) {
    Eigen::Matrix3d R = vec2SO3(rot_vec);
    Eigen::Matrix4d T = Eigen::Matrix4d::Identity();
    T.block<3, 3>(0, 0) = R;
...
int main(int argc, char**argv) {
    ...
    for (int l = 0; l < num_repeat; ++l) {
        ...
        Eigen::Vector3d rot_est(0., 0., 0.);
        Eigen::Vector3d ts_est(0., 0., 0.);
